...
Semantic handoffs can be created privately with `handoff_create(name="next.md", workspace=...)`. They are immutable records addressed by `handoff://<project-uuid>/<handoff-uuid>`, stored under XDG data; local bindings live under XDG state. Use `handoff_read(ref=...)`, `handoff_list(storage="central")`, and `handoff_search(storage="central")`. When `has_more` is true, pass the returned opaque `next_cursor` to the same operation, scope, project, and search query; central pagination does not use nonzero offsets. Non-empty central lists report `total_count=null` because catalog rows are validated lazily. The local SQLite catalog under XDG state is only a rebuildable index: portable bundles contain only the authoritative `document.md` and `manifest.json`. Rebuilding walks record identities once in O(n) time without preloading document bodies; each returned page revalidates its canonical records. `handoff_import` and `handoff_export` copy those bundles without moving sources. Workspace moves require explicit `handoff_project(..., replace=true)`. Native checkpoints remain separate recovery evidence.
