...
**Architecture:** Translate each verified Codex `ThreadItem` into the existing legacy message or function-call representation consumed by the internal migration engine. Keep private reasoning and unknown item types out of the target, with exact loss counters. Serialize structured tool results as JSON text so the engine carries them without `opaque` blocks.

**Tech Stack:** Python 3.10+ standard library, SQLite read-only projection, bundled migration engine, pytest.
...
The projector will whitelist verified item types. It will map file edits, MCP calls, collaboration calls, subagent activity, plans, hook prompts, dynamic tools, reviews, image paths/results, and compaction markers into paired `function_call` and `function_call_output` records. Remote user images remain image blocks; local media, audio, skill, and mention inputs become exact workspace references. Arguments remain JSON objects. Results become deterministic JSON text because the migration engine treats arbitrary result objects as opaque. The projector will continue to omit private reasoning and unknown item types, recording each omission.
