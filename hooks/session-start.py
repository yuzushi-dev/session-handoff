    ...
    try:
        ...
        _output(
            ...
            system_message=((notice + "\n\n") if notice else "") + (
                ...
                "`session-handoff telemetry yes` to enable optional telemetry: anonymous aggregate telemetry plus a random per-home installation ID used for registration, daily/version status observations, and managed install/uninstall counts. The registry retains the ID, first/last observation, last version, and uninstall state while the service operates. Or "
