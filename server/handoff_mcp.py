...
TOOLS = [
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "path": {"type": "string", "description": "Explicit legacy workspace file path relative to workspace, for example handoffs/2026-08-12-feature.md. Use exactly one of `path` or `name`."},
                "name": {"type": "string", "description": "Name for an immutable central record outside the workspace. Use exactly one of `name` or `path`."},
                "content": {"type": "string", "description": "Complete handoff with all canonical sections. Use exactly one of `content` or `state`."},
                "state": {**_state_schema(), "description": "Structured handoff state. Use exactly one of `state` or `content`."},
    ...
    {
        "name": "handoff_read",
        "description": "Read a handoff from inside a workspace. Provide exactly one of `path` or `ref`. Credential-like values are redacted in the returned content.",
        "inputSchema": {
            "type": "object",
            "additionalProperties": False,
            "required": ["workspace"],
            "properties": {
                "workspace": {"type": "string", "description": "Absolute workspace directory."},
                "path": {"type": "string", "description": "File path relative to workspace. Use exactly one of `path` or `ref`."},
                "ref": {"type": "string", "description": "Canonical central handoff reference. Use exactly one of `ref` or `path`."},
    ...
    },
    {
        ...
        "description": "Validate a handoff's canonical sections without changing the file. Provide exactly one of `path` or `ref`.",
        "inputSchema": {
            "type": "object",
            "additionalProperties": False,
            "required": ["workspace"],
